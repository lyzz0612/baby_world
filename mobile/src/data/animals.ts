export type AnimalCategory = 'farm' | 'zoo' | 'ocean';
...
export const CATEGORIES = {
  farm: { label: '农场', icon: '🐔', color: '#FFB84D' },
  zoo: { label: '动物园', icon: '🦁', color: '#7BC876' },
  ocean: { label: '海洋馆', icon: '🐬', color: '#6ECFF6' },
} as const;
...
export function getAnimalColor(animal: Animal): string {
  return CATEGORIES[animal.category]?.color ?? '#FFB84D';
}
