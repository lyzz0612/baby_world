...
| `KV_NAMESPACE_ID` | KV 命名空间 ID，部署时注入 `wrangler.toml` | 运行 `npx wrangler kv:namespace create CLIPBOARD_KV` 后返回的 ID |
...
2. 创建 KV 命名空间（`cloud-paste` 分支 `worker/` 目录下执行）：
   ...
   ```
   将返回的 namespace ID 添加到 GitHub Secrets 中，名称为 `KV_NAMESPACE_ID`（不要写到 `wrangler.toml`，CI 会自动注入）。
...
4. 在 GitHub 仓库的 Settings → Secrets 中添加上述三个 Secret。
